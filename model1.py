...
def train_bayes_model():
    message_spam = pd.read_csv('combined_data.csv', sep='\t',header=None, names=['Label', 'Message'])
    ...
    randomized_data = message_spam.sample(frac=1, random_state=11)
...
def classify1(message):
    # Load parameters and models
    with open('model_1_parameters.pkl', 'rb') as f:
