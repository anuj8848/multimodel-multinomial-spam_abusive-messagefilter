...
def train_bayes_model():
    message_spam = pd.read_csv('SMSSpamCollection_2', sep='\t',header=None, names=['Label', 'Message'])
    ...
    randomized_data = message_spam.sample(frac=1, random_state=65)
    ...
    training_set['Label'].value_counts().plot(kind='bar')
    plt.subplot(1, 2, 2)
    plt.title('Test Set')
    ...
    with open('model_6_parameters.pkl', 'wb') as f:
        pickle.dump((p_spam, p_ham, vocabulary, parameters_spam, parameters_ham), f)

def classify6(message):
    # Load parameters and models
    with open('model_6_parameters.pkl', 'rb') as f:
